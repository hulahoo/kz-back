...
(function (factory) {
    ...
    $.fn.orgchart = function (options) {
        var defaultOptions = {
            'nodeName': 'name',
            'nodeLastName': 'lastName',
            'nodeMiddleName': 'middleName',
            'nodeImage': 'image',
            'nodePosition': 'position',
            'nodeContact': 'contact',
            'nodeEmail': 'email',
            'nodeParentFullName': 'parentName',
            'nodeParentId': 'parentId',
            'nodeOrgName': 'organization',
            'nodeGrade': 'grade',
        ...
        var data = opts.data;
        var authorizationToken = opts.authorizationToken; //+ for rest api security
        var newScale = 100;
        var $chart = $('<div>', {
            'data': {'options': opts},
        ...
        if ($.type(data) === 'object') {
            ...
            $.ajax({
                ...
                'beforeSend': function (request) {
                    request.setRequestHeader("Authorization", "Bearer " + authorizationToken); //+ for rest api security
        ...
        var $zoomer = '<ul class="zoom-wrap-ul"><li id="minus" class="zoom-op"><a>-</a></li><li><span id="scale">100</span><span>%</span></li><li id="plus" class="zoom-op"><a>+</a></li></ul>';
        $chartContainer.append($zoomer);
        ...
        if (opts.zoom) {
            $('#minus').bind('click', function () {
                if (newScale !== 70) {
                    newScale -= 10;
                    $('#scale').text(newScale);
                    setChartScale($chart, newScale / 100);
                }
            });
    ...
    function createNode(nodeData, level, opts) {
        ...
        var parentNode = nodeData[opts.nodeParentFullName],
            $man = (parentNode !== null && parentNode !== '') ? '<div class="node-info-man">Manager: <a href="#">' + parentNode + '</a></div>' : '',
            gradeNode = nodeData[opts.nodeGrade],
            $grade = (gradeNode !== null && gradeNode !== '') ? '<div class="node-info-grade">' + nodeData[opts.nodeGrade] + '</div>' : '';

        var $info = '<div class="node-info-fn"><a data-url="team-member" class="redirect-link">' + nodeData[opts.nodeName] + ' ' + nodeData[opts.nodeMiddleName] + ' ' + nodeData[opts.nodeLastName] + '</a></div>' +
            '<div class="node-info-org"><i class="fa fa-university"></i>' + nodeData[opts.nodeOrgName] + '</div>' +
            '<div class="node-info-pos"><i class="fa fa-briefcase"></i>' + nodeData[opts.nodePosition] + '</div>' +
            $grade +
            '<div class="node-info-loc">' + nodeData[opts.nodeLocation] + '</div>' +
            '<div class="node-info-tel"><i class="fa fa-phone"></i>' + nodeData[opts.nodeContact] + '</div>' +
            '<div class="node-info-email"><i class="fa fa-envelope-o"></i>' + nodeData[opts.nodeEmail] + '</div>' +
            $man +
            '<div class="node-actions-link" style="display: none !important;"><a><i class="fa fa-cog"></i> Развернуть меню</a></div>' +
            '<div class="node-menu">' +
            '<div class="node-actions">' +
            '   <div class="node-action-title">Выбрать действие</div>' +
            '   <div class="node-action-list"><table><tr>' +
            '<td>' +
            '<ul>' +
            '<li><a data-url="hrchart$Employee.browse" class="redirect-link">Change Job and Compensation</a></li>' +
            '<li><a data-url="hrchart$Order.browse" class="redirect-link">Employment Details</a></li>' +
            '<li><a>Add: Global Assignment details</a></li>' +
            '</ul>' +
            '</td>' +
            '<td>' +
            '<ul>' +
            '<li><a>Manage alternative cost</a></li>' +
            '<li><a>Distribution</a></li>' +
            '<li><a>Generate Document</a></li>' +
            '</ul>' +
            '</td>' +
            '</tr></table>' +
            '</div>' +
            '</div>' +
            '<div class="node-redirects">' +
            '   <div class="node-redirect-title">Перейти</div>' +
            '   <div class="node-redirect-list">' +
            '<table><tr>' +
            '<td>' +
            '<ul>' +
            '<li><a>Personal Information</a></li>' +
            '<li><a>Employment information</a></li>' +
            '<li><a>Objective Plan</a></li>' +
            '</ul>' +
            '</td>' +
            '<td>' +
            '<ul>' +
            '<li><a>Compensation statements</a></li>' +
        ...
        var $nodeDiv = $('<div ' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + (nodeData.parentId ? ' data-parent="' + nodeData.parentId + '"' : '') + '>')
            ...
            .append('<div class="node-image"><img src="' + nodeData[opts.nodeImage] + '"/></div>')
            .append('<div class="title">' + nodeData[opts.nodeName] + '</div>')
            .append('<div class="node-last-name">' + nodeData[opts.nodeLastName] + '</div>')
            .append('<div class="node-position">' + nodeData[opts.nodePosition] + '</div>')
            .append('<div class="node-footer"><span>' + nodeData[opts.footerInfo] + '</span></div>')
            .append('<div class="node-info">' + $info + '</div>');
        ...
        if (opts.verticalDepth && (level + 2) > opts.verticalDepth) {
            ...
            if (Number(flags.substr(2, 1))) {
                $nodeDiv.append('<i class="edge verticalEdge bottomEdge fa"></i>')
                    .children('.node-footer').prepend('<i class="fa ' + opts.parentNodeSymbol + ' symbol"></i>');
            }
        }
        ...
        $nodeDiv.find('.node-image').on('click', function (event) {
            if (!$nodeDiv.hasClass('focused')) {
                $nodeDiv.closest('.orgchart').find('.focused').removeClass('focused');
                $nodeDiv.addClass('focused');
            } else {
                $nodeDiv.removeClass('focused');
            }
        });

        $nodeDiv.find('.node-actions-link').on('click', function (event) {
            $nodeDiv.find('.node-menu').slideToggle(200);
        });
        ...
        buildChildNode.call($node.closest('.orgchart').parent(), $node.closest('table'), data, opts, function () {
            if (++count === data.children.length) {
                ...
                if (!$node.find('.symbol').length) {
                    $node.children('.node-footer').prepend('<i class="fa ' + opts.parentNodeSymbol + ' symbol"></i>');
                }
                showChildren($node);
            }
        });
