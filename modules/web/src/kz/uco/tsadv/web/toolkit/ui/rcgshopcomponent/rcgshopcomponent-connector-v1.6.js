    ...
    function initPage() {
        ...
        if (pageName === 'shop') {
            ...
            function initOrders() {
                var filter = "",
                    $ulFilter = $('<ul />', {
                        class: 'dropdown-content select-wrapper',
                        attr: {
                            'column': 'status'
                        }
                    });
                var dicGoodsCategoryFilter = "",
                    $ulDicCategoryFilter = $('<ul/>', {
                        class: 'dropdown-content select-wrapper',
                        attr: {
                            'column': 'd.code'
                        }
                ...
                if (state.filterList.length) {
                    var filterArr = [];
                    filterArr.push('');
                    for (var j = 0; j < state.filterList.length; j++) {
                        filterArr.push(state.filterList[j]);
                    }
                    ...
                    for (var i = 0; i < filterArr.length; i++) {
                        var el = filterArr[i],
                            $li = $('<li />', {
                                ...
                                append: $('<span />', {
                                    class: 'filterable',
                                    text: messageBundle['order.input.filter.GoodsOrderStatus' + (el ? ("." + el) : "")]
                                }),
                                click: function () {
                                    var $me = $(this),
                                        $ul = $me.closest('ul'),
                                        $selectContainer = $me.closest('.select-container-rcg'),
                                        $dropdownContainer = $me.closest('.dropdown-content'),
                                        $inputDropdown = $selectContainer.find('.input-dropdown'),
                                        $span = $me.find('span');

                                    filter = "";
                                    if ($me.attr('value')) filter = "and " + $ul.attr('column') + "='" + $me.attr('value') + "'";
                                    $inputDropdown.val($span.text());
                                    $dropdownContainer.hide();

                                    currentPage = 0;
                                    recordsCount = 0;
                                    loadGoodsOrders(filter);
                                }
                            });
                        $ulFilter.append($li);
                    }
                ...
                var dicGoodsCategoryFilterArr = [];
                dicGoodsCategoryFilterArr.push('');
                // dicGoodsCategoryFilterArr.push('TRUE');
                // dicGoodsCategoryFilterArr.push('FALSE');

                var valsBykey = Object.keys(state.dicCategoryMap).map(function(key) {
                    return state.dicCategoryMap[key];
                });
                for (var value of valsBykey) {
                    dicGoodsCategoryFilterArr.push(value);
                }

                for (var v = 0; v < dicGoodsCategoryFilterArr.length; v++) {
                    var elV = dicGoodsCategoryFilterArr[v];
                    var $liValue = $('<li />', {
                        attr: {
                            value: elV
                        },
                        ...
                        click: function () {
                            var $me = $(this),
                                $ul = $me.closest('ul'),
                                $selectContainer = $me.closest('.select-container-dic-goods-category'),
                                $dropdownContainer = $me.closest('.dropdown-content.select-wrapper'),
                                $inputDropdown = $selectContainer.find('.input-dropdown-dic-goods-category'),
                                $span = $me.find('span');

                            dicGoodsCategoryFilter = "";
                            if ($me.attr('value')){
                                dicGoodsCategoryFilter = "and " +
                                    $ul.attr('column') + "=" +
                                    "'" + (getMapKeyByValue(state.dicGoodsCategory, $me.attr('value'))) + "'";
                            }
                            $inputDropdown.val($span.text());
                            $dropdownContainer.hide();
                            ...
                            currentPage = 0;
                            recordsCount = 0;
                            loadGoodsOrders(dicGoodsCategoryFilter);

                        }
                    });
                    $ulDicCategoryFilter.append($liValue);
                }

                var keys = Object.keys(state.dicCategoryMap).map(function(key) {
                    return key;
                });
                function getMapKeyByValue(map, searchValue) {
                    for (var key of keys) {
                        if (state.dicCategoryMap[key] === searchValue)
                            return key;
                    }
                }
                ...
                var $goTableBody = $('<tbody/>'),
                    $filter = $('<div />', {
                        ...
                        append: [$('<span/>', {
                            ...
                            placeholder: messageBundle['order.input.filter.placeholder'],
                            click: function (e) {
                                var $me = $(this),
                                    $container = $me.closest('.select-container-rcg');
                                $container.find('.dropdown-content').slideDown('fast');
                            }
                        }), $ulFilter]
                    }),
                    $dicGoodsCategoryFilter = $('<div/>', {
                        class: 'select-container-rcg select-container-dic-goods-category',
                        append: [$('<span/>', {
                            class: 'caret',
                            text: '▼'
                        }), $('<input/>', {
                            class: 'input-dropdown-dic-goods-category input-dropdown',
                            attr: {
                                'readonly': true
                            },
                            placeholder: messageBundle['order.input.filter.OrderDetailsVoucherUsed.placeholder'],
                            click: function (e) {
                                console.log('input click');
                                var $m = $(this),
                                    $container = $m.closest('.select-container-dic-goods-category');
                                $container.find('.dropdown-content.select-wrapper').slideDown('fast');
                            }
                        }), $ulDicCategoryFilter]
                    }),
                    $goTable = $('<table/>', {
                        ...
                        append: $('<thead/>', {
                            append: $('<tr/>', {
                                append: $('<td/>', {
                                    ...
                                    text: messageBundle['order.td.fullName']
                                })).add($('<td/>', {
                                    text: messageBundle['order.td.status']
                                })).add($('<td/>', {
                                    text: messageBundle['order.td.pdf']
                                }))
                            })
                        }).add($goTableBody)
                    }),
                ...
                var $goWrapper = $('<div/>', {
                        class: 'rcg-sh-go-w',
                        append: [$filter, $dicGoodsCategoryFilter, $goTable.add($loadMore)]
                    }),
                    $goLoading = $('<div/>', {
                ...
                function loadGoodsOrders() {
                    ...
                    $.ajax({
                        ...
                        data: {
                            ...
                            lastCount: recordsCount,
                            filter: filter,
                            dicGoodsCategory: dicGoodsCategoryFilter
                        },
                        'dataType': 'json',
                        'beforeSend': function (request) {
                ...
                function makeGoodsOrder(order) {
                    return $('<tr/>', {
                        append: $('<td/>', {
                            ...
                            text: !order.voucherQRCode ? order.fullName : '',
                            prepend: $('<i/>')
                        })).add($('<td/>', {
                            ...
                            append: $('<i/>', {
                                class: order.voucherQRCode ? 'fa fa-file-pdf-o shop-pdf' : ""
                            }),
                            click: function () {
                                if(order.voucherQRCode) {
                                    connector.printVoucherReport(order.id, order.personGroupId)
                                }
                            }
                        }))
                    });
        ...
        function initTemplate() {
            ...
            if (isShop) {
                $leftMenuUl.append($('<li/>', {
                    ...
                    text: messageBundle['my.orders'] + (goodsOrdersCount ? ' (' + goodsOrdersCount + ')' : ''),
                    click: function () {
                        activateTab(this, 2);
                    }
                }));
