 ...
 */
@Controller
public class RcgFeedbackAttachmentController {

    private MimetypesFileTypeMap mimetypesFileTypeMap = new MimetypesFileTypeMap();

    @RequestMapping(value = "rf_attachment/{id}", method = RequestMethod.GET)
    @ResponseBody
    public ResponseEntity<InputStreamResource> loadRecognitionTypeImage(@PathVariable String id) {
        ...
        try {
            ...
            FileDescriptor fileDescriptor = AppBeans.get(CommonService.class).getEntity(FileDescriptor.class, UUID.fromString(id));
            if (fileDescriptor != null) {
                ...
                if (inputStream != null) {
                    ...
                    if (fileDescriptor.getName().contains("mp4")) {
                        return ResponseEntity.ok()
                                .header("Content-Disposition", "form-data; filename=\"" + fileDescriptor.getName() + "\"")
                                .header("Content-Type", "video/mp4")
                                .header("Accept-Ranges", "bytes")
                                .header("Content-Range", "bytes")
                                .header("Content-Length", String.valueOf(inputStream.available()))
