 ...
 */
@Controller
public class RecognitionTypeImageController {

    private MimetypesFileTypeMap mimetypesFileTypeMap = new MimetypesFileTypeMap();

    @RequestMapping(value = "rcg_type_image/{code}/{count}", method = RequestMethod.GET)
    @ResponseBody
    public ResponseEntity<InputStreamResource> loadRecognitionTypeImage(@PathVariable String code, @PathVariable Long count) {
        ...
        try {
            ...
            if (!loadDefaultImage) {
                fileDescriptor = AppBeans.get(RecognitionService.class).getRecognitionTypeImage(code, count == 0 || count < -1);
            ...
            if (loadDefaultImage) {
                if (!code.matches("THANK_YOU|GOOD_JOB|OUTSTANDING")) {
                    code = "THANK_YOU";
                }

                inputStream = ServletContextHolder.getServletContext()
                        .getResourceAsStream("/VAADIN/themes/base/images/recognition/" + code + ".png");
