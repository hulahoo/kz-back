...
public class PersonGroupBrowseAllRecord extends AbstractLookup {
    ...
    private CustomFilter customFilterRemoveAllreadyAdded;
...
//        personGroupsDs.setQuery(CustomFilter.getFilteredQuery(personGroupsDs.getQuery(), (String) params.getOrDefault("personTypeFilter", "and (p.type.code not in ('CANDIDATE', 'EXEMPLOYEE') or p.type.code is null)")));
    ...
    public void ready() {
        super.ready();
        if (param.containsKey("assignmentHistoryEditKey") && param.containsKey("personGroupId")) {
            refreshDsForAssignmentHistory((UUID)param.get("personGroupId"));
        }
    }
...
//                        .addComponentAttribute("datatype", Datatypes.get(IntegerDatatype.NAME))
    ...
    private void refreshDsForAssignmentHistory(UUID personGroupId) {
        String query = "select e from base$PersonGroupExt e " +
                "join e.list l " +
                "join e.assignments a " +
                "where e.id <> :custom$personGroupId " +
                "and :custom$systemDate between l.startDate and l.endDate " +
                "and a.assignmentStatus.code = 'ACTIVE' ";
        personGroupsDs.setQuery(query);
