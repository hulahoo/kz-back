 ...
 */
@Controller
public class RecognitionGoodsImageController {

    private MimetypesFileTypeMap mimetypesFileTypeMap = new MimetypesFileTypeMap();

    @RequestMapping(value = "goods_image/{goodsId}", method = RequestMethod.GET)
    @ResponseBody
    public ResponseEntity<InputStreamResource> loadRecognitionTypeImage(@PathVariable String goodsId) {
        ...
        try {
            ...
            if (goodsId == null || goodsId.equals("") || goodsId.equalsIgnoreCase("default-goods")) {
            ...
            if (!loadDefaultImage) {
                fileDescriptor = AppBeans.get(RecognitionService.class).getRecognitionGoodsImage(goodsId);
            ...
            if (loadDefaultImage) {
                inputStream = ServletContextHolder.getServletContext()
                        .getResourceAsStream("/VAADIN/themes/base/images/recognition/default-goods.png");
