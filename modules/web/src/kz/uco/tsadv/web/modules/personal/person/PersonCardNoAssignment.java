...
/**
 * @author Anuar Manabayev
 * @since 30.01.2020
 * Личная карточка для не штатных единиц (assignmentDs == null).
 */

public class PersonCardNoAssignment extends AbstractHrEditor<PersonExt> {
                ...
                showOptionDialog(messages.getMainMessage("closeUnsaved.caption"),
                        ...
                        new Action[]{
                                ...
                                new BaseAction("discard")
                                        .withIcon(AppBeans.get(ThemeConstantsManager.class).
                                                getThemeValue("actions.dialog.Cancel.icon"))
...
//    }
    ...
    protected void initDefaultPage(String tabName) {
        if (fromAssessment) {
            ...
            openFrame(Objects.requireNonNull(linkWrappers.stream().filter(linkWrapper -> {
...
//            add(new LinkWrapper("pcf-course", "coursesTab", "font-icon:GRADUATION_CAP"));
    ...
    protected void setActive(LinkWrapper linkWrapper) {
        for (Component component : leftLinks.getOwnComponents()) {
            if (component.getClass().getSimpleName().equalsIgnoreCase("WebLinkButton")) {
                boolean selectPage = linkWrapper.getScreen() != null && component.getId() != null
                        && component.getId().equalsIgnoreCase(linkWrapper.getScreen());
    ...
    protected void initPersonLeftMenu(PersonExt person) {
        ...
        userInfoSb.append(addUserInfoTr(getMessage("Person.hireDate"), person.getHireDate() == null ?
                "" : dateFormat.format(person.getHireDate())));
        userInfoSb.append(addUserInfoTr(getMessage("Person.nationalIdentifier"),
                String.valueOf(person.getNationalIdentifier())));
        ...
        userInfoSb.append(addUserInfoTr(String.format(getMessage("Person.absenceDays"),
                new SimpleDateFormat("dd.MM.yyyy").format(CommonUtils.getSystemDate())),
                String.valueOf(absenceBalanceService.getCurrentAbsenceDays(personGroupDs.getItem()))));
        userInfoSb.append(addUserInfoTr(getMessage("totalExperience"),
                String.valueOf(employeeService.getTotalExperience(person.getGroup().getId(),
                        CommonUtils.getSystemDate()))));
        userInfoSb.append(addUserInfoTr(getMessage("experienceInCompany"),
                String.valueOf(employeeService.getExperienceInCompany(person.getGroup().getId(),
                        CommonUtils.getSystemDate()))));
        ...
        actions.add(createLinkButton("Отчет т2 форма", new BaseAction("reportT2") {
            @Override
            public void actionPerform(Component component) {
                Report report = commonService.emQuerySingleRelult(Report.class,
                        "select e from report$Report e where e.code = 'PERSON_CARD'", null);
        ...
        actions.add(createLinkButton("Редактировать", new BaseAction("edit-person") {
            @Override
            public void actionPerform(Component component) {
                AbstractEditor abstractEditor = openEditor("tsadv$PersonMainData.edit", person,
                        WindowManager.OpenType.THIS_TAB, ParamsMap.of("personDs", personDs));
        ...
        personCardLeftMenu.add(createH4("progressBarLabel", String.format("%s %.1f%s",
                getMessage("person.card.progress.label"), (float) fieldPercent, "%"), true));
