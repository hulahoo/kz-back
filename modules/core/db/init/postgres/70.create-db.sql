...
AS WITH full_hierarchy AS (
    ...
    FROM (((SELECT he.id,
                   ...
                   NULL::bigint            AS max_salary,
                   NULL::bigint            AS min_salary
            FROM base_hierarchy_element he
            WHERE ((he.delete_ts IS NULL) AND (he.element_type = 1) AND (now() >= he.start_date) AND
                   (now() <= he.end_date))
            UNION
            SELECT NULL::uuid                                                  AS uuid,
                   ...
                   (SELECT count(*) AS count
                    ...
                    WHERE ((a.position_group_id = p_1.group_id) AND
                           (a.organization_group_id = p_1.organization_group_ext_id) AND (a.delete_ts IS NULL) AND
                           (now() >= a.start_date) AND (now() <= a.end_date))) AS head_count,
                   grv.max_                                                    AS max_salary,
                   grv.min_                                                    AS min_salary
            FROM ((base_position p_1
                JOIN base_hierarchy_element he ON (((he.delete_ts IS NULL) AND (now() >= he.start_date) AND
                                                    (now() <= he.end_date) AND (he.element_type = 1) AND
                                                    (he.organization_group_id = p_1.organization_group_ext_id))))
                     ...
                     LEFT JOIN tsadv_grade_rule_value grv
                               ON ((
--                                    (grv.grade_rule_id = p_1.grade_rule_id) AND
                                       (grv.grade_group_id = p_1.grade_group_id) AND (grv.delete_ts IS NULL) AND
                                       (now() >= grv.start_date) AND (now() <= grv.end_date))))
            WHERE ((p_1.delete_ts IS NULL) AND (now() >= p_1.start_date) AND (now() <= p_1.end_date))) fh
        LEFT JOIN base_organization o ON (((o.delete_ts IS NULL) AND (now() >= o.start_date) AND
                                           (now() <= o.end_date) AND (o.group_id = fh.organization_group_id))))
             LEFT JOIN base_position p
                       ON (((p.delete_ts IS NULL) AND (now() >= p.start_date) AND (now() <= p.end_date) AND
   ...
   FROM (((SELECT rd.org_structure_request_id                       AS r_id,
           ...
           FROM (full_hierarchy fh
                    LEFT JOIN tsadv_org_structure_request_detail rd ON (((rd.delete_ts IS NULL) AND
                                                                         ((rd.change_type)::text = ANY
                                                                         ...
                                                                         (rd.element_type = fh.element_type) AND (1 =
                                                                                                                  CASE
                                                                                                                      WHEN ((rd.element_type = 1) AND
                                                                                                                          ...
                                                                                                                          THEN 1
                                                                                                                      WHEN ((rd.element_type = 2) AND
                                                                                                                            (rd.position_group_id = fh.position_group_id))
                                                                                                                          THEN 1
                                                                                                                      ELSE 0
                                                                                                                      END))))
           UNION
           ...
           FROM (tsadv_org_structure_request_detail rd
                    LEFT JOIN base_hierarchy_element he
                              ON (((he.delete_ts IS NULL) AND (now() >= he.start_date) AND (now() <= he.end_date) AND
                                   (he.organization_group_id = rd.parent_organization_group_id) AND
                                   (he.element_type = 1))))
           WHERE ((rd.delete_ts IS NULL) AND
                  (((rd.change_type)::text = 'NEW'::text) OR ((rd.change_type)::text = 'CLOSE'::text)))) t
       LEFT JOIN tsadv_grade g ON (((t.element_type = 2) AND (g.group_id = t.grade_group_id) AND
                                    (g.delete_ts IS NULL) AND (now() >= g.start_date) AND (now() <= g.end_date)))));
